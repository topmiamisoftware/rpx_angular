...
export class SettingsComponent implements OnInit {
  ...
  @ViewChild('spotbie_password_change_info_text')
  spotbiePasswordInfoText: ElementRef;
  @ViewChild('current_password_info')
  spotbieCurrentPasswordInfoText: ElementRef;
  ...
  fitBounds = false;
  locationFound = false;
  settingsForm: UntypedFormGroup = null;
  ...
  place: any;
  claimBusiness = false;
  passKeyVerificationFormUp = false;
  passKeyVerificationForm: UntypedFormGroup;
  passKeyVerificationSubmitted = false;
  businessVerified = false;
  placeToEatMediaMessage: string;
  placeToEatMediaUploadProgress = 0;
  calendlyUp = false;
  ...
  add(event: MatSelectChange): void {
    this.activeBusinessCategories = event.value;
    this.businessSettingsForm.get('originCategories').setValue(event.value);
  }
  ...
  private populateSettings(
    settingsResponse: any & {userSubscriptionPlan: BusinessMembership}
  ) {
    if (settingsResponse.success) {
      ...
      if (
        this.user.spotbie_user.user_type === AllowedAccountTypes.Unset &&
      ...
      this.settingsForm
        .get('spotbie_first_name')
        .setValue(this.user.spotbie_user.first_name);
      this.settingsForm
        .get('spotbie_last_name')
  ...
  claimThisBusiness() {
    ...
    this.userAuthService
      ...
      .subscribe(resp => this.claimThisBusinessCB(resp));
  }
  ...
  private claimThisBusinessCB(resp: any) {
    if (resp.message === 'passkey_mismatch') {
      ...
      localStorage.setItem(
        'spotbie_userType',
        this.chosenAccountType.toString()
      );
    ...
    switch (action) {
      case 'personal':
        this.settingsForm = this.formBuilder.group(settingsFormInputObj, {
          validators: [
            ValidateUsername('spotbie_username'),
        ...
        this.passwordForm = this.formBuilder.group(
          ...
          {
            validators: [
              ValidatePassword('spotbie_password'),
      ...
      case 'place_to_eat':
        this.businessSettingsForm = this.formBuilder.group({
          ...
          originTitle: ['', [Validators.required, Validators.maxLength(25)]],
          originDescription: [
            '',
            [
              Validators.required,
  ...
  startDeactivateAccount(): void {
    ...
    this.deactivationForm
      .get('spotbie_deactivation_password')
      .setValue('123456789');
  }
