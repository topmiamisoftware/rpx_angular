    ...
    this.http.post<HttpResponse>(NOTIFICATIONS_API, notifications_object, HTTP_OPTIONS)
      .subscribe( resp => {
        // console.log("Settings Response", resp);
          const notifications_response = new HttpResponse ({
          status : resp.status,
        ...
        error => {
          console.log('Friends Notifications Error : ', error);
      });
  }
  populateNotifications(notifications_response: HttpResponse) {
    if (notifications_response.status == '200') {
      // console.log("notifications_response : ", notifications_response.responseObject);
      const notifications_list = notifications_response.responseObject;
      ...
      console.log('Friends Notifications Error : ', notifications_response.httpResponse);
    }
  }
    ...
    this.http.post<HttpResponse>(NOTIFICATIONS_API, notifications_object, HTTP_OPTIONS)
      .subscribe( resp => {
        // console.log("Settings Response", resp);
          ...
          _this.acceptRequestCallback(notifications_response);
      },
      error => {
        console.log('Accept Friend Request Error : ', error);
    });
  }
  acceptRequestCallback(notifications_response: HttpResponse) {
    if (notifications_response.status == '200') {
      this.are_now_friends = true;
      this.friendship_in_q = notifications_response.responseObject;
      this.loading = false;
    } else {
    ...
    this.http.post<HttpResponse>(NOTIFICATIONS_API, notifications_object, HTTP_OPTIONS)
      .subscribe( resp => {
        // console.log("Settings Response", resp);
          ...
          _this.declineRequestCallback(notifications_response);
      },
      error => {
        console.log('Friends Decline Request Error : ', error);
    });
  }
  declineRequestCallback(notifications_response: HttpResponse) {
    if (notifications_response.status == '200') {
      this.are_now_friends = true;
      this.friendship_in_q = notifications_response.responseObject;
      this.loading = false;
