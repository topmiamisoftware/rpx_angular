...
@Component({
  ...
  styleUrls: ['../friends/friends.component.css', './pending-friends.component.css']
})
export class PendingFriendsComponent implements OnInit {
  ...
  public initCallPending(){
    ...
    .subscribe( resp => {
      ...
      this.callPendingCallback(httpResponse);
    },
      error => {
  ...
  private callPendingCallback(httpResponse : HttpResponse){
    if(httpResponse.status == "200"){      
      ...
      this.loading = false;
      //console.log("your pending friends are : ", this.pending_list);    
    } else {
      console.log("Error callPendingCallback : ", httpResponse);
    }
  }
