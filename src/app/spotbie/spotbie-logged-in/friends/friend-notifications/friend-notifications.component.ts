...
export class FriendNotificationsComponent implements OnInit {
  ...
  fetchFriendsNotifications() {
    ...
    this.http.post<any>(NOTIFICATIONS_API, notifications_object, HTTP_OPTIONS)
      .subscribe( resp => {
  ...
  populateNotifications(notifications_response: any) {
  ...
  acceptRequestCallback(notifications_response: any) {
    if (notifications_response.status == '200') {
  ...
  declineRequestCallback(notifications_response: any) {
