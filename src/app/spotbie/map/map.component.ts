...
export class MapComponent implements OnInit {
  ...
  @ViewChild('businessHomeDashboard') businessHomeDashboard: BusinessDashboardComponent
  ...
  surroundingObjectList = new Array()
  ...
  toggleReailShopsInfo(){
    this.reatailShop = !this.reatailShop
  }

  toggleUsersAroundYouInfo(){
  ...
  classificationSearchCallback(httpResponse: any){
    ...
    if(httpResponse.success){
      const classifications: Array<any> = httpResponse.data._embedded.classifications
  ...
  apiSearch(keyword: string, resetEventSorter: boolean = false) {
    this.loading = true
    ...
    switch(this.searchCategory){
      case 'events':
        ...
        // Retrieve the SpotBie Community Member Results
        this.locationService.getSpotBieCommunityMemberList(searchObjSb).subscribe(resp => {
  ...
  spawnCategories(obj: any): void {
    ...
    if(this.searchCategory !== undefined) {
      this.previousSeachCategory = this.searchCategory
    }
  ...
  getEventsSearchCallback (httpResponse: any): void {
    ...
    if(httpResponse.success){
      ...
      for (let i = 0; i < eventObjectList.length; i++) {
        ...
        const timeHr = new TimeFormatPipe().transform(eventObjectList[i].dates.start.localTime)
  ...
  ticketMasterLargestImage(imageList: any){
    const largestDimension = Math.max.apply(Math, imageList.map(
  ...
  saveUserLocation(): void {
    ...
    if(this.isLoggedIn === '1'){
      this.locationService.saveCurrentLocation(save_location_obj).subscribe(resp => {
  ...
  retrieveSurroudings(){
    ...
    this.locationService.retrieveSurroudings(retrieveSurroundingsObj).subscribe(resp => {
