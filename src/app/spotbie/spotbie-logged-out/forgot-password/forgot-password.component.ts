...
export class ForgotPasswordComponent implements OnInit {
  ...
  step_three = false;
  step_four = false;
  pin_ready_msg: string = DEF_PIN_PASS_OR_EM_MSG;
  email_or_ph_error: string = DEF_INC_PASS_OR_EM_MSG;
  new_password_msg: string = NEW_PASS_MSG;
  token: string;
  userEmail: string;
  ...
  private initForgotPassForm(): void {
    const spotbie_email_or_ph_validators = [
      Validators.required,
  ...
  get spotbie_email_or_ph() {
    return this.passwordResetForm.get('spotbie_email_or_ph').value;
  }
  get g() {
  ...
  setPassResetPin(): void {
    ...
    const emailOrPhone = this.spotbie_email_or_ph;
